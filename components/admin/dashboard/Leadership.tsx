...
import { Plus, Trash2, X, User, RefreshCcw, Link, Upload } from 'lucide-react';
...
const Leadership = ({ searchQuery = '' }: { searchQuery?: string }) => {
    ...
    const handleUrlApply = () => {
        ...
        setFormData(prev => ({ ...prev, avatar: trimmed }));
        toast.success('Image URL applied');
    };

    const resetForm = () => {
        setFormData({ name: '', role: '', desc: '', specialization: '', avatar: '' });
        setEditingId(null);
        setImageFile(null);
        setUrlInput('');
    ...
    const handleEdit = (member: any) => {
        ...
        setImageFile(null);
        setUrlInput(member.avatar?.startsWith('http') ? member.avatar : '');
        // Auto-select URL mode if existing avatar is a URL
        setImageInputMode(member.avatar?.startsWith('http') ? 'url' : 'upload');
        setShowForm(true);
    };
                                    ...
                                    {imageInputMode === 'upload' && (
                                        <div className="relative flex-grow">
                                            ...
                                            <label
                                                ...
                                                className="flex items-center justify-center gap-3 w-full bg-white/5 border border-white/10 border-dashed rounded-2xl px-4 py-12 cursor-pointer hover:bg-white/10 hover:border-emerald-500/50 transition-all min-h-[160px] relative overflow-hidden"
                                            >
                                                <div className="text-center">
                                                    <Upload size={24} className="mx-auto mb-2 text-white/20" />
                                                    <p className="text-sm font-bold text-white">
                                                        {imageFile ? imageFile.name : formData.avatar && !formData.avatar.startsWith('http') ? 'Change Portrait Image' : 'Click to upload portrait'}
                                                    </p>
                                        ...
                                        <div className="space-y-3">
                                            <div className="flex gap-2">
                                                <input
                                                    ...
                                                    className="flex-1 bg-white/5 border border-white/10 rounded-xl px-4 py-2.5 text-white text-sm focus:outline-none focus:border-emerald-500 transition-all placeholder:text-white/20"
                                                    placeholder="https://example.com/photo.jpg"
                                                />
                                                <button
                                                    type="button"
                                                    onClick={handleUrlApply}
                                                    className="px-4 py-2.5 bg-emerald-500 text-black rounded-xl font-bold text-xs uppercase tracking-widest hover:bg-emerald-400 transition-all whitespace-nowrap"
                                                >
                                                    Apply
                                                </button>
                                            </div>
                                            <p className="text-[10px] text-white/30 px-1">
                                                Paste a direct image URL (JPG, PNG, WebP). Press Apply or Enter to preview.
                                            </p>
                                        </div>
                                    )}

                                    {/* Preview — shown for both modes when avatar is set */}
                                    {formData.avatar && (
                                        <div className="relative rounded-2xl overflow-hidden border border-white/10 bg-white/5">
                                            ...
                                            <div className="absolute top-2 right-2 flex gap-2">
                                                {editingId && formData.avatar !== originalImage && (
                                                    <button
                                                        type="button"
                                                        onClick={removeImage}
                                                        className="flex items-center gap-1 px-2.5 py-1.5 bg-black/70 backdrop-blur-sm rounded-lg text-[10px] text-red-400 hover:text-red-300 font-bold uppercase tracking-widest transition-colors border border-white/10"
                                                    >
                                                        <RefreshCcw size={10} />
                                                        Reset
                                                    </button>
                                                ...
                                                <button
                                                    ...
                                                    className="flex items-center gap-1 px-2.5 py-1.5 bg-black/70 backdrop-blur-sm rounded-lg text-[10px] text-white/60 hover:text-white font-bold uppercase tracking-widest transition-colors border border-white/10"
                                                >
                                                    <X size={10} />
                                                    Clear
                                                </button>
                                            ...
                                            <div className="absolute bottom-0 left-0 right-0 px-3 py-2 bg-gradient-to-t from-black/80 to-transparent">
                                                <p className="text-[10px] text-emerald-400 font-bold uppercase tracking-widest">✓ Preview</p>
                                            </div>
                                        </div>
                                    )}
                                </div>
